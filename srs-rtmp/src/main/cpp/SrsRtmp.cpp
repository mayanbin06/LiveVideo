...
int SrsRtmp::SendAacData(uint8_t* data, int len, long timestamp) {
  ...
  // see srs_librtmp.h
  char SrsAudioCodecIdAAC = 10;

  // 2 = 22 kHz
  char sound_rate = (int)(sample_rate_ / (11 * 1000));
  // 1 = 16-bit samples
  char sound_size = sample_format_ == 16 ? 1 : 0;
  // 1 = Stereo sound
  char sound_type = sound_type_;

  int ret = 0;
  if ((ret = srs_audio_write_raw_frame(
      rtmp_, SrsAudioCodecIdAAC, sound_rate,
  ...
  LOGI("sent packet: type=%s, time=%d, size=%d, codec=%d, rate=%d, sample=%d, channel=%d", 
      srs_human_flv_tag_type2string(SRS_RTMP_TYPE_AUDIO), timestamp, len, SrsAudioCodecIdAAC,
      sound_rate, sound_size, sound_type);
  return ret;
}

void SrsRtmp::SetAudioParams(int sample_rate, int channel_count, int sample_format, int sound_type) {
